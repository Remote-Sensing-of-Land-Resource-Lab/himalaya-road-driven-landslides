...
output_tif = r"H:\Himalaya\RF_susceptibility\susceptibility\b.result_map\LSM_2020_2024_5foldcv.tif"
...
def predict_large_raster():
    ...
    try:
        for feat in feature_names:
            if "Litho" in feat or "LC_" in feat:
                continue 
            ...
            for window in tqdm(windows, desc="Processing"):
                ...
                for i, fname in enumerate(feature_names):
                    ...
                    if fname.startswith("Litho_"):
                        class_id = int(float(fname.split("_")[1]))
                        X_block[:, i] = (data_lith.reshape(-1) == class_id)
                        
                    elif fname.startswith("LC_"):
                        class_id = int(fname.split("_")[1])
                        X_block[:, i] = (data_lc.reshape(-1) == class_id)
