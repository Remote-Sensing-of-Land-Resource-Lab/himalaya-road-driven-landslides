# elevation_NDVI_road_sensitivity_with_cache_and_heatmap.py
# -*- coding: utf-8 -*-

# Features:
# 1. Further stratify by NDVI within different elevation bins;
# 2. Train an independent RF within each elevation x NDVI subgroup and calculate road sensitivity using the subgroup's own perturbations;
# 3. If an output CSV already exists, read the results directly and plot to avoid retraining the RF;
# 4. Draw a 2D road sensitivity heatmap for NDVI x elevation;
...
output_dir = r"H:\Himalaya\figure\NDVI_Elevation_2D_independent_RF"
os.makedirs(output_dir, exist_ok=True)

output_csv = os.path.join(output_dir, "NDVI_elevation_binned_RF_RoadSensitivity.csv")
output_heatmap_fig = os.path.join(output_dir, "NDVI_elevation_RoadSensitivity_heatmap.png")
output_heatmap_pdf = os.path.join(output_dir, "NDVI_elevation_RoadSensitivity_heatmap.pdf")
...
n_ele_bins = 5
ele_bin_edges = [0, 1000, 2000, 3000, 4000, 5000]
...
def sort_result_df(res_df):
    ...
    if "ele_center" not in res_df.columns:
        res_df["ele_center"] = res_df["ele_bin"].apply(interval_center)
    ...
    res_df["ele_center"] = pd.to_numeric(res_df["ele_center"], errors="coerce")
    ...
    res_df = res_df.dropna(subset=["ele_bin", "ndvi_bin", "ndvi_center"]).copy()

    res_df = res_df.sort_values(
        ["ele_center", "ndvi_center"]
...
def plot_ndvi_elevation_sensitivity_heatmap(
    res_df,
    output_png,
    output_pdf=None,
    x_bin_col="ndvi_bin",
    ...
    title="NDVI-elevation road sensitivity",
    annotate=False,
    cmap="BrBG",
...
# =========================
# 5. elevation x NDVI sensitivity 计算函数
# =========================

def run_elevation_analysis():
   
    print("\n" + "=" * 80)
    print(f"Running analysis: NDVI_elevation ({col_ele})")
    print("=" * 80)
    ...
    if os.path.exists(output_csv) and not force_recompute:
        ...
        if "ele_bin_left" in res_df.columns and "ele_bin_right" in res_df.columns:
            ele_edges_used = np.array(
                sorted(
                    set(pd.to_numeric(res_df["ele_bin_left"], errors="coerce").dropna().tolist()) |
                    set(pd.to_numeric(res_df["ele_bin_right"], errors="coerce").dropna().tolist())
                ),
                dtype=float
            )
        else:
        ...
        print(f"Loaded {len(res_df)} valid elevation x NDVI sensitivity records.")
        return res_df, ele_edges_used
    ...
    if ele_bin_edges is None:
        df["ele_bin"], ele_bin_edges_used = pd.qcut(
            df[col_ele],
            q=n_ele_bins,
            retbins=True,
            duplicates="drop"
        ...
        ele_bin_edges_used = np.array(ele_bin_edges_used, dtype=float)

    else:
        ele_bin_edges_used = np.array(ele_bin_edges, dtype=float)

        if sorted(ele_bin_edges_used) != list(ele_bin_edges_used):
            raise ValueError("ele_bin_edges must be in ascending order.")

        if len(np.unique(ele_bin_edges_used)) != len(ele_bin_edges_used):
            raise ValueError("ele_bin_edges contain duplicated values.")

        df["ele_bin"] = pd.cut(
            df[col_ele],
            bins=ele_bin_edges_used,
            include_lowest=True
        )

    actual_ele_bins = df["ele_bin"].nunique()

    if actual_ele_bins < n_ele_bins:
        print(
            f"Warning: requested {n_ele_bins} elevation bins, "
            f"but only {actual_ele_bins} bins were created. "
            f"This may be caused by duplicated values near bin boundaries."
        )

    print("\nelevation bin edges:")
    print(ele_bin_edges_used)

    print("\nelevation bin sample counts:")
    print(df["ele_bin"].value_counts().sort_index())
    ...
    df = df.dropna(subset=["ele_bin", "ndvi_bin"]).copy()

    print(f"\nSamples after elevation and NDVI binning: {len(df)}")

    results = []

    print(
        f"\nTraining Random Forest within each elevation x NDVI bin "
        f"(1:1 downsampling repeated {n_resamples} times)..."
    )

    group_cols = ["ele_bin", "ndvi_bin"]

    for (ele_interval, ndvi_interval), sub in df.groupby(group_cols, observed=False):
        ...
        if n_total_before < min_samples_per_bin:
            print(
                f"Skip elevation {ele_interval}, NDVI {ndvi_interval}: "
                f"too few samples before balancing ({n_total_before})"
            )
            results.append({
                "analysis_name": "NDVI_elevation",
                "ele_bin": str(ele_interval),
                "ele_bin_left": ele_interval.left,
                "ele_bin_right": ele_interval.right,
                "ele_center": 0.5 * (ele_interval.left + ele_interval.right),
        ...
        try:
            ...
            print(
                f"elevation {ele_interval}, NDVI {ndvi_interval}: "
            ...
            print(
                f"Skipped elevation {ele_interval}, NDVI {ndvi_interval} "
    ...
    if len(res_df) == 0:
        raise RuntimeError("No valid elevation x NDVI bins for analysis.")
    ...
    return res_df, ele_bin_edges_used


# =========================
# 6. Run elevation x NDVI analysis and plot a 2D heatmap
# =========================

res_df, ele_edges_used = run_elevation_analysis()

print("\nValid elevation x NDVI bins used:")
print(res_df[[
    "ele_bin",
    "ndvi_bin",
    "ele_center",
...
plot_ndvi_elevation_sensitivity_heatmap(
    res_df=res_df,
